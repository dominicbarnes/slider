...
Handle.prototype.onmousemove = function (e) {
    var pos = e.clientX - slider.track.getBoundingClientRect().left;
...
Handle.prototype.position2value = function (position) {
    var s = this.slider,
        val = (position / (s.width() - this.width() * 2)) * (s._max - s._min) + s._min;
...
Handle.prototype.value2position = function (value) {
    ...
    return ((val - s._min) / (s._max - s._min)) * (s.width() - this.width() * 2);
};

/**
 * Takes a value and "rounds" it to the nearest value that corresponds to the
...
Handle.prototype.value = function (value) {
    if (typeof value === "undefined") {
        ...
        if (slider.handles && slider.handles.length === 2) {
            left = slider.handles[0];
            right = slider.handles[1];
